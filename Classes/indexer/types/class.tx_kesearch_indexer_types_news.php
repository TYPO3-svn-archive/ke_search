...
class tx_kesearch_indexer_types_news extends tx_kesearch_indexer_types {
	...
	public function getAttachedContentElements($newsRecord) {
		...
		if (TYPO3_VERSION_INTEGER >= 6002000) {
			$addWhere = \TYPO3\CMS\Backend\Utility\BackendUtility::BEenableFields('tt_content') . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause('tt_content');
			$newsVersion = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::getExtensionVersion('news');
		} else {
			$addWhere = t3lib_befunc::BEenableFields('tt_content') . t3lib_befunc::deleteClause('tt_content');
			$newsVersion = t3lib_extMgm::getExtensionVersion('news');
		}

		// since version 3.2.0 news does not use a mm-table anymore for attached
		// content elements
		if (version_compare($newsVersion, '3.2.0') >= 0) {
	...
	public function getContentFromContentElements($contentElements) {
		...
		if (count($contentElements)) {
			...
			if (TYPO3_VERSION_INTEGER >= 6002000) {
				...
				/* @var $pageIndexerObject tx_kesearch_indexer_types_page  */
				$pageIndexerObject = t3lib_div::makeInstance('tx_kesearch_indexer_types_page', $this->pObj);
			}
